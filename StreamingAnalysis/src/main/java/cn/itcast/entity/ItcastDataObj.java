...
/**
 * 根据kafka存储的json字符串定义对应的javaBean对象
 */
@Data
public class ItcastDataObj {
     ...
     * kafka存储的是json字符串，flink消费出来以后需要写入到hdfs中，最终需要hive加载hdfs的数据，实现hdfs-》hive的同步
     * 因此要求写入到hdfs文件的数据格式一定是能够被hive所识别的字符串结构，而这个方法就是拼接字符串
     * @return string
     */
